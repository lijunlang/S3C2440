...
#define DEVICE_NAME     "s3c2440_led"
...
static int s3c2440_led_ioctl (struct inode *inode, struct file *filp, unsigned int cmd, unsigned int arg)
{
	printk("DOT buffer is %x\n" ,arg>>8 );
	printk("DOT buffer is %x\n" ,arg );
    switch(cmd){
        case LED_DIG_IOCTRL:
            	writel (arg >> 8, led_base + 0x10) ;   
            	writel (arg >> 8 | arg << 16,led_base + 0x11) ;   
  	return readl(led_base + 0x10);
...
static ssize_t s3c2440_led_write(struct file *filp, const char *buf, size_t count, loff_t * f_pos)
{
	int i;
...
static struct file_operations s3c2440_led_fops = {
    owner:THIS_MODULE,
...
int __init s3c2440_led_init(void)
{
    int ret;	
    led_base = ioremap(LED_BASE, 0x20);
	...
	devno = MKDEV(DEVICE_MAJOR, DEVICE_MINOR);
        mycdev = cdev_alloc();
        cdev_init(mycdev, &s3c2440_led_fops);
        err = cdev_add(mycdev, devno, 1);
        if (err != 0)
           printk("s3c2440 motor device register failed!\n");
 
       myclass = class_create(THIS_MODULE, "s3c2440-led");
       if(IS_ERR(myclass)) {
...
void __exit s3c2440_led_exit(void)
{
      	cdev_del(mycdev);
